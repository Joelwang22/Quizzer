...
const joinLine = (line) =>
  normalizePunctuationSpacing(normalizeWhitespace(line.map((entry) => entry.text).join(' ')));
...
const extractPageLines = (items) => {
  ...
  const left = positioned.filter((item) => item.x < splitX);
  const right = positioned.filter((item) => item.x >= splitX);

  const hasTwoColumns = left.length > positioned.length * 0.25 && right.length > positioned.length * 0.25;
...
if (!inputArg || !outputArg) {
  console.error('Usage: node scripts/extract-pdf-columns.mjs <input.pdf> <output.txt>');
  process.exit(1);
}

...
const StoryConversationPlayer = ({
  ...
  useLayoutEffect(() => {
    ...
    const updatePosition = (): void => {
      ...
      const preferredBubbleMaxWidth = Math.max(180, Math.min(isCompact ? 272 : 384, stageRect.width - 24));
      const expandedBubbleMaxWidth = Math.max(preferredBubbleMaxWidth, stageRect.width - 24);
      const activeBubbleMaxWidth = bubbleMaxWidth ?? preferredBubbleMaxWidth;
      const availableHeightAboveSpeaker = Math.max(60, anchorRect.top - stageRect.top - 20);

      if (bubbleRect.height > availableHeightAboveSpeaker && activeBubbleMaxWidth < expandedBubbleMaxWidth - 1) {
        setBubbleMaxWidth(expandedBubbleMaxWidth);
        return;
      }
  ...
  }, [activeLine, activeSpeakerId, bubbleMaxWidth, isCompact]);
  ...
  return (
    <div className={isCompact ? '' : `rounded-[1.75rem] p-6 shadow-[0_20px_70px_rgba(15,23,42,0.18)] sm:p-7 ${shellClassName}`}>
      {isCompact ? (
        <div className="flex items-start justify-between gap-3">
          ...
          {isComplete ? (
            <button
              ...
              className="shrink-0 rounded-lg border border-slate-600 bg-slate-900/70 px-3 py-2 text-xs font-semibold text-slate-100 hover:bg-slate-800"
            >
              Replay scene
            </button>
          ) : null}
      ...
      <div
        className={
          isCompact
            ? 'mt-4'
            : 'mt-5 rounded-[1.5rem] border border-slate-700/80 bg-slate-950/50 px-4 py-5 sm:px-5 sm:py-6'
        }
      >
        <div
          ref={stageRef}
          className={`relative ${
            isCompact ? 'min-h-[17.25rem]' : 'min-h-[21rem]'
          } ${
            hasUserCharacter
              ? `grid grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] ${isCompact ? 'gap-3 sm:gap-4' : 'gap-4 sm:gap-6'}`
              : `flex items-end justify-evenly ${isCompact ? 'gap-3 sm:gap-5' : 'gap-4 sm:gap-8'}`
          }`}
        >
          {activeLine ? (
            <div
              ref={activeBubbleRef}
              className={`absolute z-20 w-max rounded-[1.35rem] border border-teal-300/55 bg-slate-900/95 shadow-[0_12px_30px_rgba(15,23,42,0.24)] ${
              ...
              style={{
                ...
                maxWidth: `${bubbleMaxWidth ?? (isCompact ? 272 : 384)}px`,
              }}
            >
              <p
                className={`font-semibold ${
                  isCompact ? 'text-xs' : 'text-sm'
                } ${storyCastById.get(activeSpeakerId ?? '')?.accentClassName ?? 'text-slate-100'}`}
              >
                {storyCastById.get(activeSpeakerId ?? '')?.name ?? ''}
              </p>
              <p className={`mt-1 text-slate-100 ${isCompact ? 'text-xs leading-6' : 'text-sm leading-7'}`}>
                {activeLine.text}
              </p>
              <div
                  ...
                  <div className="flex h-full w-full flex-col justify-end items-start text-left">
                    ...
                    >
                      <CastSprite
                        ...
                        size={isCompact ? 56 : 96}
                        unstyled
                      />
                      <p className={`mt-2 font-semibold ${isCompact ? 'text-xs' : 'text-sm'} ${leftStageMember.accentClassName}`}>
                        {leftStageMember.name}
                      </p>
                  ...
                  return (
                    <div key={member.id} className="flex flex-col items-end justify-end text-right">
                      ...
                      >
                        ...
                        <p className={`mt-2 font-semibold ${isCompact ? 'text-xs' : 'text-sm'} ${member.accentClassName}`}>
                          {member.name}
                        </p>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          {!isComplete ? (
            <button
              type="button"
              onClick={handleNext}
              className={`rounded-lg border border-slate-600 bg-slate-900/70 font-semibold text-slate-100 hover:bg-slate-800 ${
          ...
          {!isComplete ? (
            <button
              type="button"
              onClick={handleSkip}
              className={`rounded-lg border border-slate-700 bg-transparent font-semibold text-slate-300 hover:bg-slate-900/60 ${
                isCompact ? 'px-3 py-2 text-xs' : 'px-4 py-2 text-sm'
              }`}
            >
              Skip to end
          ...
          {isComplete && showFullScriptToggle ? (
            <button
              type="button"
        ...
        {isComplete && showFullScript && showFullScriptToggle ? (
  ...
  return (
    <div className="mb-7 border-b border-slate-800 pb-6">
      <StoryConversationPlayer
        ...
        eyebrow="Northwind Scene"
        badge="Check Setup"
        cast={cast}
        lines={storyCheck.setupLines}
        takeawayTitle=""
        takeaway=""
...
const SlideCheck = ({
  ...
  const showPresentationTitle = !(storyCheck && presentation.mode === 'multiple_choice');
  ...
  return (
    <div className={`${baseSlidePanelClass} px-8 py-8 sm:px-10 sm:py-9`}>
      ...
      <div className={`${showPresentationTitle ? 'mt-7' : 'mt-3'} w-full`}>
